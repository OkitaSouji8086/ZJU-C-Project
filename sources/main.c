...
void CharEventProcess(char c)
{
	...
	if (CurrMode != EDIT || !isSelected || c == 3 || c == 22)
		return;
...
void MouseEventProcess(int x, int y, int button, int event)
{
	...
	case BUTTON_DOWN:
		...
		if (CURR_OBJ_KIND == -1)
			break;
		((ptr_StartBox)CURR_OBJ)->Color = SELCOLOR;
...
void CaseF5Process()
{
	...
	isInText = TRUE;
	((ptr_StartBox)CURR_OBJ)->Color = EDITCOLOR;
}

void CopyObj()
{
	if (CURR_OBJ == NULL || isInText)
		return;
...
void PasteObj()
{
	...
	((ptr_StartBox)PastedObj)->Color = SELCOLOR;
